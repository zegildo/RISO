...
public class RISOPos {
	...
	public static void main(String args[]) throws IOException{
		...
		//levando em consideracao exatamente a palavra sem nenhuma variação
		for (int j = 0; j < 10; j++) {
			LeitorArquivo leitor = new LeitorArquivo("pos/PartOfSpeech"+(j+1)+".txt");
			leitor.geraPalavrasArquivo();
			List<String> conceitos = leitor.getPalavras();
			FileOutputStream file = new FileOutputStream(new File("pos/done/PartOfSpeech"+(j+1)+"_Done.txt"));
			int identificados = 0;
